...
function calculateTrigramSimilarity(queryTrigramsArray, targetTrigramsArray) {
...
async function handleAdapterEvent(message, sender) {
  const { type: type2, payload } = message;
  if (!payload) return;
  if (type2 === "CONVERSATION_UPDATE") {
    await upsertConversationBase(payload, message);
    return;
  }
  if (type2 === "PROMPT_SUBMITTED") {
    ...
    const mergedKeywords = Array.from(
      new Set(
        [...existingKeywords, ...newKeywords].map((k) => String(k || "").trim()).filter(Boolean)
      )
    ).slice(0, 16);
    await db.conversations.update(base.id, {
    ...
    chrome.runtime.sendMessage({ type: "CONVERSATION_UPDATED", payload: { conversationId: base.id } }).catch(() => {
    });
...
async function upsertTagsFromKeywords(keywords) {
  ...
  await Promise.all(
    keywords.map((k) => String(k || "").trim()).filter(Boolean).map((label) => label.toLowerCase()).map((label) => db.tags.put({ id: label, label }))
  );
}
